...
def match_model_weights(model, newmodel, cfg_mask, weight_model=None, cfg=None):
    layer_id_in_cfg = 0
    ...
    newmodel_ind = 0
    # for [m1, m_weight] in zip(newmodel.modules(), weight_model.modules()):
    for [cfg_block, m_weight] in zip(cfg, weight_model.modules()):
        if cfg_block == 0:
            continue
        m1 = newmodel[newmodel_ind]
